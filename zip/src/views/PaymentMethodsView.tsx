...
export const PaymentMethodsView = ({ paymentMethods, setPaymentMethods, setView, showAlert, showConfirm }: any) => {
    ...
    const [editingPM, setEditingPM] = useState({ id: '', name: '', icon: 'payments' });
    ...
    return (
        <div className="space-y-4 flex flex-col h-full">
            <div className="flex items-center justify-between mb-4 shrink-0">
                ...
                <button onClick={() => { setEditingPM({ id: '', name: '', icon: 'payments' }); setModalOpen(true); }} className="text-primary"><Icon name="add_circle" size="text-3xl" /></button>
            </div>
                    ...
                    <div key={pm.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex justify-between items-center group">
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center shrink-0">
                                <Icon name={pm.icon} size="text-xl" />
                            </div>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <button onClick={() => { setEditingPM(pm); setModalOpen(true); }} className="p-1 text-gray-400 hover:text-primary"><Icon name="edit" size="text-xl"/></button>
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-4 gap-3">
                            {availableIcons.map(icon => (
                                <div key={icon} onClick={() => setEditingPM({...editingPM, icon})} className={`cursor-pointer flex justify-center items-center py-3 rounded-lg border-2 transition-all ${editingPM.icon === icon ? 'border-primary bg-primary/10 text-primary' : 'border-gray-100 bg-gray-50 text-gray-400 hover:bg-gray-100'}`}>
